...
const PastryBot = () => {
  ...
  const [chatOpen, setChatOpen] = useState(false); // New state to toggle chat window visibility
  ...
  const toggleChat = () => {
    setChatOpen((prev) => !prev); // Toggle the chat window visibility
  };
  ...
  return (
    <Container sx={{ display: "flex", justifyContent: "center", p: 3 }}>
      {chatOpen ? (
        ...
        <IconButton
          onClick={toggleChat} // Open the chat when clicked
          sx={{
            position: "fixed",
            bottom: "20px",
            right: "20px",
